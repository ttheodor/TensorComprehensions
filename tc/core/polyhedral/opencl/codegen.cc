...
#include "tc/core/polyhedral/opencl/codegen.h"
#include "tc/core/polyhedral/opencl/mapping_types.h"
...
string emitTypedTensorName(
  ...
  stringstream ss;
  ss << (constInput ? "const " : "") << "__global "
     << halideTypeString(t.type()) << "* " << makePointerName(t.name());
  return ss.str();
}
...
void emitArgs(stringstream& ss, const Scop& scop) {
  ...
  auto sigVec = emitTypedTensorNames(scop.halide.outputs);
...
void emitKernelSignature(
  ...
  ss << "__kernel void " << specializedName << "(";
  emitArgs(ss, scop);
  ss << ") {" << endl;
}
...
void emitTensorView(
  ...
  ss << ws.tab();
  ss << (constInput ? "const " : "") << " __global " << halideTypeString(p.type()) << " (*"
...
struct LdgWrapper {
 public:
  LdgWrapper(const CodegenStatementContext& context, isl::id id)
      : readOnly_(false), out_(context.ss) {}
...
void emitThreadIdInit(stringstream& ss, const MappedScop& scop) {
  WS ws;
  ss << ws.tab();
  ss << "unsigned long b0 = get_local_id(0); unsigned long b1 = get_local_id(1); unsigned long b2 = get_local_id(2);\n";
  ss << ws.tab();
  ss << "unsigned long t0 = get_group_id(0); unsigned long t1 = get_group_id(1); unsigned long t2 = get_group_id(2);\n";
}
